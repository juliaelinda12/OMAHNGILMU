<?php
    ...
    if (isset($_GET["id"])) {
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM quote WHERE id_quote='$id'");

        while ($row = mysqli_fetch_array($result)) {
            
		$id_quote = $row['id_quote'];
  		$nama_quoter = $row['nama_quoter'];
  		$isi_quote = $row['isi_quote'];
  
        }
    }
    else {
        header("location:quoteDashboard.php");
	}
    ...
    <div class="sidebar row no-gutters mt-5">
        <div class="col-md-2 bg-dark mt-2 pt-4" height="100%">
            <ul class="flex-column">
                ...
                <li class="nav-item">
                    <a class="nav-link warna" href="quoteDashboard.php"><i class="fa-solid fa-quote-left me-3"></i>Quotes</a><hr class="bg-secondary">
                </li>
            </ul>
        </div>
                ...
                <p>Bersama kami wujudkan Indonesia melek literasi</p>
                    <div class="card me-2">
                        ...
                        <div class="card-body mt-2">
                            <form action="ProsesEditQuote.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
                                <div class="row g-3">
                                    <div class="col-sm-6 form-group mx-auto">
                                        <label for="id_quote">Id Quote</label>
                                        <input type="number" name="id_quote" id="id_quote" value="<?php echo $id_quote;?>" class="form-control" required readonly>
                                    </div>
                                    <div class="col-sm-6 form-group mx-auto">
                                        <label for="nama_quoter">Nama Quote</label>
                                        <input type="text" name="nama_quoter" id="nama_quoter"  value="<?php echo $nama_quoter;?>" class="form-control" required>
                                    </div>
                                    <div class="col-sm-12 form-group mx-auto">
                                        <label for="isi_quote">Isi Quote</label>
                                        <textarea type="text" name="isi_quote" id="isi_quote" class="form-control" placeholder="Tambahkan isi_quote"><?php echo $isi_quote;?></textarea>
                                    </div>
                                    <div class="col-12">
                                    ...
                                    <button class="btn btn-warning text-light" type="submit" name="edit" id="edit" value="Edit Quote">Edit Quote</button>
                                </div>
                            </form>
                        </div>
                    </div>
